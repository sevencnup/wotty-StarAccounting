...
import { navigateDocument, resolveDocumentNavigationHref } from "@/lib/document-navigation";
...
export default function RegisterPage() {
  ...
  const loginHref = resolveDocumentNavigationHref("/auth/login");
  const homeHref = resolveDocumentNavigationHref("/");
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      navigateDocument(dashboardRoutePath, { replace: true });
    } catch (submitError) {
      ...
      <div className="grid w-full max-w-5xl gap-6 lg:grid-cols-[minmax(0,1.15fr)_420px]">
        ...
        <ThemeSurface className="p-6 sm:p-8">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-slate-500">填写基础信息，创建你的 wotty 账户。</p>
          </div>
          ...
          <div className="mt-5 flex items-center justify-between text-sm text-slate-500">
            <a className="hover:text-slate-900 hover:underline" href={loginHref}>
              去登录
            </a>
            <a className="hover:text-slate-900 hover:underline" href={homeHref}>
              返回首页
            </a>
          </div>
        </ThemeSurface>
