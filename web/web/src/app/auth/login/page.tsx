...
function LoginForm() {
  ...
  const next =
    nextParam && nextParam.startsWith("/") && !nextParam.startsWith("/auth") && nextParam !== "/"
      ? nextParam
      : dashboardRoutePath;
  const registerHref = resolveDocumentNavigationHref("/auth/register");
  ...
  useEffect(() => {
    ...
    setRedirecting(true);
    navigateDocument(next, { replace: true });
  }, [next]);
  ...
  return (
    <div className="relative min-h-dvh overflow-hidden bg-[linear-gradient(180deg,#f7fbff_0%,#eef4fb_52%,#e9eff8_100%)]">
      <div className="hidden lg:block"><WaveBackdrop /></div>
          ...
          <ThemeSurface className="relative border-white/75 bg-white/92 p-6 shadow-[0_24px_64px_rgba(15,23,42,0.1)] backdrop-blur sm:p-8">
            ...
            <div className="relative">
              ...
              <p className="mt-4 text-sm text-slate-500">
                还没有账户？
                <a className="ml-1 font-medium text-slate-950 transition hover:text-blue-700" href={registerHref}>
                  立即注册
                </a>
              </p>
