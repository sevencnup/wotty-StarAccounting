      ...
      <BottomSheet open={isScheduleOpen} onOpenChange={setIsScheduleOpen}>
        <BottomSheetContent className="max-w-3xl">
          ...
          <ThemeTable className="max-h-[60vh] overflow-auto">
            {schedule.length === 0 ? (
              <div className="px-4 py-10 text-center text-base" style={{ color: "var(--theme-muted-text)" }}>当前贷款已全部结清。</div>
            ) : (
              <table className="w-full min-w-[560px] text-sm sm:text-base">
                <thead className="sticky top-0 text-left">
                  <tr>
                    <th className="px-4 py-3 font-medium" style={{ background: "var(--theme-dialog-section-bg)", color: "var(--theme-muted-text)" }}>期数</th>
                    <th className="px-4 py-3 font-medium" style={{ background: "var(--theme-dialog-section-bg)", color: "var(--theme-muted-text)" }}>预计还款日</th>
                    <th className="px-4 py-3 text-right font-medium" style={{ background: "var(--theme-dialog-section-bg)", color: "var(--theme-muted-text)" }}>应还金额</th>
                    <th className="px-4 py-3 text-right font-medium" style={{ background: "var(--theme-dialog-section-bg)", color: "var(--theme-muted-text)" }}>剩余本金</th>
                  </tr>
                </thead>
                <tbody>
                  {schedule.map((row) => (
                    <tr key={row.index} className="border-t" style={{ borderColor: "var(--theme-surface-border)" }}>
                      <td className="px-4 py-3" style={{ color: "var(--theme-muted-text)" }}>第 {row.index} 期</td>
                      <td className="px-4 py-3" style={{ color: "var(--theme-body-text)" }}>{row.date}</td>
                      <td className="px-4 py-3 text-right font-medium" style={{ color: "var(--theme-body-text)" }}>¥{row.amount.toFixed(2)}</td>
                      <td className="px-4 py-3 text-right" style={{ color: "var(--theme-muted-text)" }}>¥{row.remaining.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
