...
export const PAGE_META: Record<string, PageMeta> = {
  ...
  "/about/": { title: "关于项目", subtitle: "查看产品说明与版本信息。" },
};

export function resolveNavigationHref(href: string, themeId: ThemeId) {
  return normalizeNavigationPath(href === "/" ? getDashboardRoutePath(themeId) : href);
}

export function isNavigationItemActive(href: string, pathname: string) {
  return href === "/" ? isDashboardRoutePath(pathname) : normalizeNavigationPath(pathname) === normalizeNavigationPath(href);
}
...
export function getPageMeta(pathname: string): PageMeta {
  ...
  return PAGE_META[normalizeNavigationPath(pathname)] ?? {
    title: "财务面板",
