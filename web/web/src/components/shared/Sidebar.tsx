...
import { useEffect, type ComponentType, type MouseEvent as ReactMouseEvent, type PointerEvent as ReactPointerEvent } from "react";
import Link from "next/link";
import { usePathname, useRouter } from "next/navigation";
import { ChevronDown, Moon, Sun } from "lucide-react";
import { cn } from "@/lib/utils";
import { navigateDocument } from "@/lib/document-navigation";
import { flushNavigationDebug, persistNavigationDebug } from "@/components/shared/navigation-debug";
import { NAV_ITEMS, isNavigationItemActive, resolveNavigationHref } from "@/components/shared/navigation";
import { warmNavigationRoute } from "@/components/shared/navigation-warmup";
...
function shouldHandleDocumentNavigation(event: ReactMouseEvent<HTMLAnchorElement>) {
...
function useWarmNavigationProps(themeId: ThemeId, pathname: string) {
  const router = useRouter();

  useEffect(() => {
    flushNavigationDebug("sidebar");
  }, []);

  return (href: string, isActive: boolean, onNavigate?: () => void) => ({
    href,
    prefetch: false,
    onPointerDownCapture: (event: ReactPointerEvent<HTMLAnchorElement>) => {
      persistNavigationDebug("sidebar", "pointerdown", {
        href,
        pathname,
        isActive,
    ...
    onClickCapture: (event: ReactMouseEvent<HTMLAnchorElement>) => {
      persistNavigationDebug("sidebar", "click-capture", {
        href,
        pathname,
        isActive,
    ...
    onClick: (event: ReactMouseEvent<HTMLAnchorElement>) => {
      ...
      if (!shouldHandleDocumentNavigation(event)) {
        persistNavigationDebug("sidebar", "skip-modifier", {
          href,
          pathname,
          isActive,
          button: event.button,
      ...
      if (isActive) {
        persistNavigationDebug("sidebar", "skip-active", {
          href,
          pathname,
        });
        return;
      ...
      persistNavigationDebug("sidebar", "navigate", {
        href,
        pathname,
        isActive,
      });
    ...
    onMouseEnter: () => warmNavigationRoute({ href, themeId, router }),
    onFocus: () => warmNavigationRoute({ href, themeId, router }),
    onTouchStart: () => {
      persistNavigationDebug("sidebar", "touchstart", {
        href,
        pathname,
        isActive,
      });
...
export function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const getWarmNavigationProps = useWarmNavigationProps(themeId, pathname);
    ...
    >
      <div className="px-1 pb-5">
        ...
        >
          ...
          <span className="sr-only">wotty</span>
        </div>
      </div>
        ...
        <div className="space-y-1.5">
          {primaryItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                {...getWarmNavigationProps(href, isActive, onNavigate)}
