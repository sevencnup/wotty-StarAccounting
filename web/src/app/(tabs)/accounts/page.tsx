...
export default function AccountsPage() {
  ...
  const visibleAccounts = useMemo(() => (
    summary.positive.filter((item) => filter === "ALL" || item.type === filter)
  ), [summary.positive, filter]);
    ...
    <div className="page-stack finance-page accounts-page">
      ...
      <section className="account-wallet-hero">
        ...
        <div className="account-wallet-stats">
          <div><span>平均余额</span><strong>¥ {formatMoney(summary.average)}</strong></div>
      ...
      <section className="account-browser-section">
        ...
        <div className="account-card-grid">
          {visibleAccounts.length ? visibleAccounts.map((asset) => {
            ...
            const activity = accountActivity(asset, transactions);
            ...
            return (
              <article className={`account-wallet-card ${meta.className}`} key={asset.id}>
                ...
                <div className="account-wallet-balance">¥ {formatMoney(asset.balance)}</div>
                <div className="account-wallet-foot"><span>{activity.count} 笔匹配流水</span><time>{updatedLabel(activity.latest)}</time></div>
              </article>
            );
      ...
      <section className="home-card finance-section account-focus-section">
        ...
        <div className="account-focus-list">
          {summary.topThree.map((asset, index) => (
            <div key={asset.id}><span><i>{index + 1}</i>{asset.name}</span><strong>¥ {formatMoney(asset.balance)}</strong></div>
          ))}
