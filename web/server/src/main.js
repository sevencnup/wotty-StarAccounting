...
function getUpdateManifestSourceUrls() {
    ...
    return [
        "https://raw.githubusercontent.com/xingseven/wotty-StarAccounting/main/web/public/updates/latest.json",
    ];
}
...
function aggregatePlatformExpenseMetrics(rows, groupBy = "day") {
    const map = new Map();
    for (const row of rows) {
        const date = row.date instanceof Date ? row.date : new Date(row.date);
        if (Number.isNaN(date.getTime()))
        ...
        if (!Number.isFinite(amount))
            continue;
        const current = map.get(key) ?? { wechat: 0, alipay: 0, cloudpay: 0, unknown: 0 };
        current[platformKey] += amount;
        map.set(key, current);
    }
    return Array.from(map.entries())
        .sort((a, b) => a[0].localeCompare(b[0]))
...
function buildConsumptionDashboard(rows, bucket = "day") {
    ...
    const platformTrendYearly = aggregatePlatformExpenseMetrics(rows.map((row) => ({
        date: row.date,
        amount: row.amount,
...
function buildDashboardOverviewConsumption(rows, bucket = "month") {
    const base = buildConsumptionDashboard(rows, bucket);
    const topMerchant = base.merchants[0];
    const repeatMerchantMap = new Map();
    for (const row of rows) {
        if (row.type !== "EXPENSE")
            continue;
        const amount = Number(row.amount ?? 0);
        if (!Number.isFinite(amount))
            continue;
        const merchant = row.merchant?.trim() || "未知商户";
        const current = repeatMerchantMap.get(merchant) ?? { total: 0, count: 0 };
        current.total += amount;
        current.count += 1;
        repeatMerchantMap.set(merchant, current);
    }
    const repeatMerchantTotal = Array.from(repeatMerchantMap.values())
        .filter((item) => item.count >= 2)
        .reduce((sum, item) => sum + item.total, 0);
    return {
        summary: {
            totalExpense: base.summary.totalExpense,
            totalIncome: base.summary.totalIncome,
        },
        trend: base.trend.slice(-6),
        platformDistribution: base.platformDistribution.slice(0, 4),
        insights: {
            concentration: {
                topMerchant: topMerchant?.merchant ?? "暂无",
                topMerchantShare: base.summary.totalExpense > 0 && topMerchant ? (topMerchant.total / base.summary.totalExpense) * 100 : 0,
        ...
        transactions: base.transactions.slice(0, 6),
    };
}
...
app.get("/api/dashboard/overview-consumption", async (req, res) => {
    const { start, end, search } = parseQuery(req);
    ...
    if (prisma) {
        try {
            const where = buildPrismaTransactionWhere({ userId, accountId, start, end, search });
            ...
            jsonOk(res, buildDashboardOverviewConsumption(rows, bucket));
            return;
        }
        catch (e) {
            const message = e instanceof Error ? e.message : "unknown";
    ...
    const rows = (transactionsByAccount.get(accountId) ?? transactionsByUser.get(userId) ?? []).map((item) => ({
        id: item.id,
        date: item.date,
        type: item.type,
        amount: item.amount,
    ...
    const filtered = rows.filter((row) => {
        ...
        if (end && ts > end.getTime())
            return false;
        if (!matchesTransactionSearch(row, search))
            return false;
        return true;
    });
    jsonOk(res, buildDashboardOverviewConsumption(filtered, bucket));
});
