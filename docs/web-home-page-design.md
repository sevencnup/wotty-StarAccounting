...
- **最近记账改用共享行**：`最近记账表格里，金额怎么不靠右了` → 此前首页为 4 列专用布局，与消费页 5 列不同；改为直接复用消费页同一个 `.recent-row` 5 列组件，金额回到最后一列贴右。
- **预算预警进度条统一风格**：`预算预警和里的进度条样式做成和储蓄贷款的样式一样如何` → 预警条高度 7px→8px、底色对齐，填充由纯色改为与储蓄/贷款一致的横向渐变（safe 绿 / warn 琥珀 / danger 红各一档渐变）。
...
7. 最近记账直接复用消费页同一个 `.recent-row` 5 列布局（分类徽标 + 标题 + 类别 + 时间 + 金额），保证两页一致、金额始终贴右。
8. 进度条玻璃化：储蓄/贷款与预算预警进度条统一为 8px 高 + 横向渐变填充 + 顶部微光罩层（`::after`），呼应 hero 液态玻璃。
