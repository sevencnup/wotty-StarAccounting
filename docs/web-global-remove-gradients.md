...
- `web/src/app/globals.css`：核心样式表中的所有渐变规则清洗与统一。
- `web/src/components/**/*.tsx` 及 `web/src/app/**/*.tsx`：包含内联样式 `background: linear-gradient(...)` 或渐变色定义的组件。

## 3. 实施步骤

- [x] 步骤 1：排查全局 CSS 与 TSX/JSX 代码中所有的 `gradient` 出现位置。
- [x] 步骤 2：对全局背景、卡片背景、按钮、Tab 选项、选中态、进度条等分类制定纯色替换方案。
- [x] 步骤 3：修改 `web/src/app/globals.css`，去除所有渐变定义。
