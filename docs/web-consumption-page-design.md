...
- `app/src/app/globals.css`：`.calendar-day` 系列、`.calendar-grid/.calendar-week-row` 间距、`.sankey-scroll::after`。
- `app/src/components/stark/ConsumptionCharts.tsx`：`buildTrendOption` 的 yAxis `splitLine` 由 `{ show: false }` 改为淡虚线；`buildSankeyOption` 的 `left` 由 8px 增到 22px，缓解左侧微信节点贴边。
- `app/package.json`：版本 0.0.63 → 0.0.65。
...
4. 唯一 console 报错为 `api-sync` 12367 连接失败，与页面无关。
5. 收据 Hero 保持原样未动。
